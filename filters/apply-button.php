...
<?php var_dump(remove_duplicates(esc_url($apply_url))); ?>
    <a class="wpc-filters-submit-button" href="<?php echo remove_duplicates(esc_url($apply_url)); ?>"><?php
         $button_text = isset($set['apply_button_text']['value']) ? esc_html($set['apply_button_text']['value']) : esc_html__('Apply', 'filter-everything');
         echo $button_text;
         ?></a>
    <a class="wpc-filters-reset-button" href="<?php echo remove_duplicates(esc_attr($reset_url)) ?>"><?php
