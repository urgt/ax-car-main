...
<div class="<?php echo flrt_filter_class($filter); // Already escaped ?>"
    ...
    <div class="<?php echo esc_attr(flrt_filter_content_class($filter)); ?>">
        <?php if (!empty($terms) || $view_args['ask_to_select_parent']): ?>
            <select
                id="wpc-<?php echo esc_attr($filter['entity']); ?>-<?php echo esc_attr($filter['e_name']); ?>-<?php echo esc_attr($filter['ID']); ?>"
                ...
                <?php if ($view_args['ask_to_select_parent'] !== false): ?>
                    <option class="wpc-dropdown-default" value="0" data-wpc-link="<?php echo esc_attr($noSelectUrl); ?>"
                        id="wpc-option-<?php echo esc_attr($filter['entity']); ?>-<?php echo esc_attr($filter['e_name']); ?>-0"><?php
                                 echo $current_language == 'en' ?  'Select brand first' :  'Сначала выберите бренд';
                                  ?></option>
                <?php else: ?>
                    <option<?php echo esc_html($data_default); ?> class="wpc-dropdown-default" value="0"
                        data-wpc-link="<?php echo esc_attr($noSelectUrl); ?>"
                        id="wpc-option-<?php echo esc_attr($filter['entity']); ?>-<?php echo esc_attr($filter['e_name']); ?>-0"><?php
                                  if (pll_current_language() == 'en') {
                                      echo esc_html(pluralize($filter['label']));
                                  } else {
                                      echo $filter['label'];
                                  }
                                  ;
                        ...
                        foreach ($terms as $id => $term_object) {
                            ...
                            ?>
                            <option<?php echo esc_html($data_image);
                            echo esc_html($data_color);
                            echo esc_html($data_count); ?>
                                class="wpc-term-count-<?php echo esc_attr($term_object->cross_count); ?> wpc-term-id-<?php echo esc_attr($term_object->term_id);
                                      echo esc_attr($term_hidden_class); ?>"
                                value="<?php echo esc_attr($term_object->term_id); ?>" <?php selected(1, $selected); ?>             <?php disabled(1, $disabled); ?>
                                data-wpc-link="<?php echo esc_attr($url_manager->getTermUrl($term_object->slug, $filter['e_name'], $filter['entity'])); ?>"
                                id="wpc-option-<?php echo esc_attr($filter['entity']); ?>-<?php echo esc_attr($filter['e_name']); ?>-<?php echo esc_attr($id); ?>"><?php
